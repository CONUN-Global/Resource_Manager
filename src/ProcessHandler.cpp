...
ProcessHandler::ProcessHandler(Config *c)
{
    ...
    if( OpenMutex(MUTEX_ALL_ACCESS, 0, this->m_aMutexName) )
    {
        if(this->m_cfg->GetClose())
        {
            if(CreateMutex(0, true, this->m_dMutexName) != NULL)
            {
                ...
                {
					fprintf(stdout, "Close the Resource_Management attached by the process id: %d.\n", this->m_cfg->GetProcessId());
                }
                else
                {
                    fprintf(stdout, "Close the Resource_Management attached by the process name: %s.\n", this->m_cfg->GetExeName());
                }
        ...
        {
            fprintf(stderr, "Resource_Management already started!\n");
        }
        this->m_error += 1;
    }
    ...
    if(!this->m_error)
    {
        ...
        if(this->m_cfg->GetHighPriority())
        {
            ...
            {
                if(this->m_cfg->GetVerbose()) fprintf(stdout, "Priority changed to %d for Resource_Management.\n", HIGH_PRIORITY_CLASS);
            }
            else
            {
...
int ProcessHandler::CheckState()
{
    ...
    {
        printf("Target process %d is Resource_Management itself!\nAborting because it makes no sense.\n", this->m_Id);
        state = 0;
    }
...
char * ProcessHandler::GenerateMutexName(int mode)
{
    ...
    if(this->m_Id)
    {
        sprintf_s(mutex_name, 250, "cpulimit_MODE-%d_PROCESSID-%d", mode, this->m_Id);
    }
    else if (this->m_cfg->GetExeName())
    {
        sprintf_s(mutex_name, 250, "cpulimit_MODE-%d_PROCESSNAME-%s", mode, this->m_cfg->GetExeName());
    }
    else
    {
        sprintf_s(mutex_name, 250, "cpulimit_MODE-%d", mode);
    }
	...
	strcpy_s(out, name_size, mutex_name);
